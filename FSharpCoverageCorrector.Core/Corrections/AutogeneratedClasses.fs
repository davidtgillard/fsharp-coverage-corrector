...
let private groupAutoGeneratedClasses (pkg: Package) =
  ...
  let updatedAutoGenGroups = (seq {
    for nf in notFound do
      let _, _, autoGenClasses, _ = nf
      let firstLine = autoGenClasses.Head.Lines[0] // we know that autoGenClasses is not empty, and Lines is also not empty, so this is safe
      ...
      | Some grp -> yield {grp with AutoGeneratedClasses = autoGenClasses @ grp.AutoGeneratedClasses}
      | None -> 
        let autogeneratedClassNames = autoGenClasses |> List.map (fun c -> c.Name) |> String.concat "; "
        failwith $"No matching original method found for autogenerated classes: {autogeneratedClassNames}"
  } |> Seq.toList)
  
  // concat the update groups and the found groups, throwing away any found groups that were updated
  (updatedAutoGenGroups @ foundGroups) |> List.distinctBy (fun c -> c.ClassName, c.MethodOfClass)
...
let private correctMethod (grp: MethodAutoGeneratedGroup) (package: Package) =
  ...
  let updatedClass = Class.Create(origClass.Name, origClass.SourceFile, replace (fun m1 m2 -> m1.Name = m2.Name) updatedMethod origClass.Methods, origClass.NonMethodLines)
